		...
		if (ctx->label_do) {
			...
			case TIME_VALUE_SAMPLE_RATE:
				g_string_append_printf(*out, "%s%s",
					ctx->label_names ? "Time (sample_rate)" : ctx->xlabel,
					ctx->value);
				break;
			case TIME_VALUE_NOW_ABS:
				g_string_append_printf(*out, "%s%s",
					ctx->label_names ? "Time (now_abs)" : ctx->xlabel,
					ctx->value);
				break;
			case TIME_VALUE_NOW_REL:
				g_string_append_printf(*out, "%s%s",
					ctx->label_names ? "Time (now_rel)" : ctx->xlabel,
					ctx->value);
				break;
			}
...
static void save_gnuplot(struct context *ctx)
{
	...
	for (i = 0; i < num_channels; i++) {
		...
		g_string_append_printf(script, "using %u:($%u * %g + %g), ",
			(ctx->time > TIME_VALUE_FALSE), i + 1 + (ctx->time > TIME_VALUE_FALSE), ctx->scale ?
